...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type = str, default='../checkpoint/domainet', help='path to save the checkpoint')
    parser.add_argument('--data', type = str, default= 'clipart', help='[clipart | infograph | painting | quickdraw | real | sketch]')
    args = parser.parse_args()

    exp_folder = 'single_domainnet'
    ...
    log = args.log
    if log:
        log_path = os.path.join('../logs/domainet/', exp_folder)
    ...
    trainset = DomainNetDataset(data_base_path, args.data, transform=transform_train)
    testset = DomainNetDataset(data_base_path, args.data , transform=transform_test, train=False)
    # subset sample
